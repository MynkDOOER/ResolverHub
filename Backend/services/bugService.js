...
export const getBug = async (bugId, userId) => {
	...
	return await bug.populate("reportedBy", "name");
};
...
export const updateBug = async (bugId, bugData, userId) => {
	...
	if (user.role === "Tester") {
		if (user._id.toString() !== bug.reportedBy.toString()) {
			throw new Error("Unauthorized");
		}
		...
		if (bugData.description)
			allowedUpdates.description = bugData.description;
		...
		if (!["InProgress", "Resolved"].includes(bugData.status)) {
			throw new Error(
				"Developer can only set status to In Progress and Resolved",
			);
		}
